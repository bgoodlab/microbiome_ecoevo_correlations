...
#
###############################################################################
def parse_sample_metadata_map(HMP_ONLY=True):
#def parse_sample_metadata_map2(): 
    
    sample_metadata_map = {}
    
    # First load HMP metadata
    #file = open(config.scripts_directory+"HMP_ids_order.txt","r")
    file = open("HMP1-2_ids_order.txt","r")
    file.readline() # header
    for line in file:
        items = line.split("\t")
    ...
    # Then load Twin data
    file = open("twin_ids_order.txt","r")
    file.readline() # header
    for line in file:
        items = line.split("\t")
        subject_id = items[0].strip()
    ...
    file = open("korpela_twin_ids.txt","r")
    file.readline() # header
    for line in file:
        items = line.split("\t")
        subject_id=items[0].strip()
...
def print_sample_metadata():
    ...
    outFile=open(os.path.expanduser("~/ben_nandita_hmp_analysis/sample_metadata_supplement.txt"),'w')
   ...
   # Then load kuleshov data
    ...
    file.readline()
    for line in file:
        outFile.write(line.strip() +'\tXie et al\n') 
...
def list_of_isolates_and_mixtures():
    ...
    for sample_id in isolate_metadata_map:
        subject_id, dummy, accession_id, country, continent, order = isolate_metadata_map[sample_id]
        if country=='isolate':
